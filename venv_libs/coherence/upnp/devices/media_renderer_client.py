...
class MediaRendererClient(EventDispatcher, log.LogAble):
    ...
    def __init__(self, device):
        ...
        if self.av_transport:
            ...
            self.av_transport.service.subscribe_for_variable(
                'AVTransportURI', 0, self.state_variable_change)
            self.av_transport.service.subscribe_for_variable(
    ...
    def state_variable_change(self, variable):
        if variable.old_value != variable.value :
            self.s = '----->  %(name)r changed from %(old_value)r to %(value)r'
            self.info(self.s,vars(variable))
